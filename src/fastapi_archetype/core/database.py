...
def is_local_dev_mode(settings: AppSettings | None = None) -> bool:
    ...
    return cfg.effective_database_url.startswith(SQLITE_PREFIX)
...
def get_engine(settings: AppSettings | None = None) -> Engine:
    ...
    if _engine is None:
        ...
        if effective.startswith(SQLITE_PREFIX):
            _engine = create_sqlite_engine(effective, echo=cfg.debug)
        else:
            try:
                make_url(effective)
            except Exception as e:
                raise ValueError(f"Invalid DATABASE_URL: {effective!r}. {e!s}") from e
            _engine = create_url_engine(effective, echo=cfg.debug)
    return _engine
