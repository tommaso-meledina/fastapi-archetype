...
- **Client identity (UUID vs ID):** Entities that need a stable, client-facing identifier have a `uuid` property (string, UUID format). Response DTOs expose `uuid` and **never** expose the internal persistence `id`. This keeps persistence details hidden and gives clients a stable reference for updates and links. For update operations (e.g. PUT), the route accepts the UUID in both the path and the request body; if they do not match, the API returns **400 Bad Request**. The factory or service resolves the entity by UUID to obtain the internal ID, then maps the request DTO to the entity for update.
...
3. **Factory:** Create `factories/widget.py` with `entity_to_dto(entity) -> GetWidgetsResponse` (or the appropriate response type) and `dto_to_entity(dto: PostWidgetsRequest) -> Widget` using only Pydantic (`model_validate`, `model_dump`). For update (PUT): when mapping request DTO to entity, resolve the entity by UUID (e.g. fetch by `uuid` to get the persistence row), then build the entity from the DTO using the resolved `id` and `uuid` — the client never sends `id`.
