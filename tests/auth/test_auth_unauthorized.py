...
from .conftest import mock_facade_unauthorized, mock_facade_unexpected_error
...
class TestUnauthorizedSanitization:
    ...
    def test_malformed_token_does_not_leak_provider_details(
        ...
        app.dependency_overrides[get_auth_facade] = mock_facade_unauthorized
        try:
    ...
    def test_unexpected_error_does_not_leak_urls(
        ...
        app.dependency_overrides[get_auth_facade] = mock_facade_unexpected_error
        try:
            response = entra_client.post(
                "/test/auth-required",
                json={"value": "X"},
