...
def test_v2_logs_on_list(client: TestClient, caplog: logging.LogRecord) -> None:
    with caplog.at_level(logging.INFO, logger="fastapi_archetype.api.v2.dummy_routes"):
        client.get("/v2/dummies")
    assert any("v2 list_dummies returned" in r.message for r in caplog.records)
